...
public class ReadTestData  {
		...
		try {
			...
			for (int i = 1; i <= rows; i++) {
				...
				if (sCurTestCaseName.equalsIgnoreCase(sTestCaseName)) {
					...
					// Below Loop is used to get all the headers names in the
					// excel
					for (int j = 0; j < cols; j++) {
						sKey = sh.getRow(0).getCell(j).getStringCellValue().trim();
						sValue = "";
						if (sh.getRow(i).getCell(j) != null) {
							if (sh.getRow(i).getCell(j).getCellTypeEnum() == CellType.STRING) { //.getCellType
	...
	public static Map<String, String> retrieveData(String sheetName, String testCaseName) {
		...
		try {
			testData = readTestData(BaseClass.excelPath, sheetName, testCaseName);
	...
	/*public static void main(String[] args) {
		...
		retrieveData("TestData", "TC001");//Here you have to pass the sheet and TC name
	}*/
		...
		try {
			if (fileName.length() > 1 && !fileName.isEmpty() && !fileName.equals(null) && testCaseName.length() > 1
				...
				Object obj = new JSONParser()
						.parse(new FileReader(System.getProperty("user.dir") + "/TestData/" + fileName + ".json"));
	...
	/*public static void main(String[] args) {
		...
		Map<String, String> testData = getJsonData("JsonData", "TC001");
		
		System.out.println(testData.get("JobID"));
	}*/
}
